...
Properties {
    ...
    $NuGetApiKey = $null
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $EncryptedApiKeyPath = "$env:LOCALAPPDATA\vscode-powershell\$PublishRepository-NuGetApiKey.clixml"
}
...
Task StoreKey -requiredVariables EncryptedApiKeyPath {
    $null = PromptUserForNuGetApiKeyCredential -DestinationPath $EncryptedApiKeyPath
    "The NuGetApiKey has been stored in $EncryptedApiKeyPath"
}
